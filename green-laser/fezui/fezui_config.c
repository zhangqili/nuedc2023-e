...
void fezui_timer_handler()
{
    ...
    sprintf(fezui_buffer,"x:%.0f",phi_pid.errdat);
    u8g2_DrawStr(&(fezui.u8g2), 0, 8*7, fezui_buffer);

    sprintf(fezui_buffer,"y%.0f",theta_pid.errdat);
    u8g2_DrawStr(&(fezui.u8g2), 0, 8*8, fezui_buffer);
